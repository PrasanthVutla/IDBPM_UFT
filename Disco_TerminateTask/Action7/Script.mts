...
Browser("title:=BPM Console").Page("title:=BPM Console").WebElement("class:=v-window-header","innertext:=New Disconnect Request").WaitProperty "innertext", "New Disconnect Request", 1000
Browser("title:=BPM Console").Page("title:=BPM Console").WebElement("class:=v-window-header","innertext:=New Disconnect Request").CheckProperty "innertext", "New Disconnect Request", 100
...
'WorkRequest Number
Browser("title:=BPM Console").Page("title:=BPM Console").WebEdit("Class Name:=WebEdit","html id:=workrequestnumberId").Set DataTable.Value("TerminateWR_No")

Browser("title:=BPM Console").Page("title:=BPM Console").WebElement("innertext:=Start","class:=v-menubar-menuitem").WaitProperty "visible", True, 1000
Browser("title:=BPM Console").Page("title:=BPM Console").WebElement("innertext:=Start","class:=v-menubar-menuitem").CheckProperty "innertext", "Start", 100
Wait 15
...
If Browser("title:=BPM Console").Page("title:=BPM Console").WebElement("Class Name:=WebElement","class:=v-loading-indicator second v-loading-indicator-delay").Exist Then
	...
	wait 10
	Reporter.ReportEvent micPass, "Disconnect Task Created - Successfully for Terminate", "Create Task Successful"
	Else
		Reporter.ReportEvent micFail, "Disconnect Task Created - Failed", "Create Task Un-Successful"
End If
